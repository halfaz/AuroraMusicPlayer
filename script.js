...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!localStorage.getItem('serverIP')) {
        ...
        serverAddress = localStorage.getItem('serverIP');
        fetchMusicList();
    }
...
async function fetchMusicList() {
    try {
        const response = await fetch('http://${serverAddress}/music/');
...
function playSong(index) {
    ...
    audioPlayer.src = `http://${serverAddress}/music/${song.name}`;
...
let serverAddress = localStorage.getItem('serverIP') || 'localhost:3000';

function showIPConfig() {
    const ipModal = document.getElementById('ipConfig');
...
async function fetchMusicList() {
    try {
        ...
        console.error('Error fetching music:', error);
        showIPConfig(); // Show IP config if connection fails
    }
}
